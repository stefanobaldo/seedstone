...
# on the refusal — it raises — so a pair at redis-py 8 reaches this server
# only by naming the protocol it will speak. Measured on redis-py 8.1.0
# against seedstone 0.1.1: bare, the handshake fails; with this parameter,
# every command below runs. See ../../README.md.
_QUERY = "db=0&protocol=2"
_SERVER = (
    "redis://:%s@%s:%s?%s" % (_PASSWORD, _HOST, _PORT, _QUERY)
    if _PASSWORD
    else "redis://%s:%s?%s" % (_HOST, _PORT, _QUERY)
)
